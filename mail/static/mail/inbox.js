...
function openEmail(email, mailbox) {
    ...
    fetch(`/emails/${email.id}`)
        ...
        .then(mail => {
            ...
            from.innerHTML = `From: <span> <input disabled class="form-control" value="${mail.sender}"> at ${mail.timestamp} </span>`

            const body = document.createElement("div")
            body.className = 'form-group, mail-header'
            body.innerHTML = `Msg: <textarea disabled class="form-control ">${mail.body}</textarea>`
